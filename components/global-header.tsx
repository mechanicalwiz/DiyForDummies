...
function DesktopMenuLinks() {
  return (
    <Stack d={['none', 'flex', 'flex']} shouldWrapChildren isInline spacing='15px' alignItems='center' color='gray.50'
      ...
      <Link ml='10px' bgGradient='linear(to-l, cyan.900, green.600)' p='7px 10px' rounded='6px'
            _hover={{ textDecoration: 'none', bgGradient: 'linear(to-l, green.700, cyan.900)' }}
            fontWeight={500} href={'/signup'}>Pay me :D</Link>
    </Stack>
  );
}
...
export function GlobalHeader(props: GlobalHeaderProps) {
  ...
  return (
    <Box bg={variant === 'solid' ? 'red.900' : 'transparent'} p='20px 0'>
        ...
        <Flex justifyContent='space-between' alignItems='center'>
          <Box>
            <Link w='100%'
              ...
              <Text color="chartreuse" d={['block', 'none', 'block']} as='span'>DIY KITS FOR LOSERS</Text>
            </Link>
          </Box>
          <DesktopMenuLinks />
          <MobileMenuLinks />
